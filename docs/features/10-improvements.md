...
### Wave 42 — Path C: Logprob-based classifier confidence (in progress)
...
| # | Tag | SHA | What landed | Tests |
|---|---|---|---|---:|
| 1 | (doc)         | this    | Wave 42 mid-wave (start) — register Wave 42 in §10.5 + §10.6 with the logprob-based classifier confidence design + Path C framing + post-Wave-41 backlog promotion. Updates §10.1 wave-status + §10.4 reservation row for IMPROVE-179. | 0 |
| 2 | [IMPROVE-179] | pending | NEW `logprobs: bool = False` + `top_logprobs: int \| None = None` fields on `GenerationSettings` (additive) + MODIFIED `OllamaProvider.chat()` + `OllamaProvider.achat()` to pass them to the Ollama client when set + NEW `dag_classifier_logprobs_enabled` env-var setting + NEW `_compute_logprob_confidence(response)` helper in `executor.py` + MODIFIED `classify_llm_router_edges` to prefer logprob-based confidence when env-var on + logprobs present, fall back to W33 heuristic otherwise. 7 NEW behaviour tests across `tests/test_providers_base.py` + `tests/test_ollama.py` + `tests/test_dag_classifier_confidence.py` (settings-shape / settings-from-dict / provider-passes-logprobs / provider-omits-logprobs / classifier-uses-logprobs / classifier-falls-back-on-missing / classifier-falls-back-on-disabled). | +7 |
| 3 | (doc)         | pending | Wave 42 end-wave retrospective. Adds ✓ prefix on §10.4 IMPROVE-179 row. Fills in mid-wave + numbered SHA placeholders. Flips Wave 42 status (in progress → ✓ shipped). NEW Wave 42 architectural impact subsection. | 0 |

Net: +7 Tier 1 "passed" tests (2017 → 2024 vs the
documented Wave-41-close baseline). Sweep file count 103
...
W40 + W41; the twelfth single-numbered wave shape in this
run.
...
|---|---|---|---|---:|
| 1 | (doc)         | this    | Wave 42 mid-wave (start) — register Wave 42 in §10.5 + §10.6 with the logprob-based classifier confidence design + Path C framing + post-Wave-41 backlog promotion. Updates §10.1 wave-status + §10.4 reservation row for IMPROVE-179. | 0 |
| 2 | [IMPROVE-179] | pending | NEW `logprobs: bool = False` + `top_logprobs: int \| None = None` fields on `GenerationSettings` (additive defaults preserve all existing callers) + MODIFIED `OllamaProvider.chat()` + `OllamaProvider.achat()` to pass them to the Ollama Python client v0.6.1 when set + NEW `dag_classifier_logprobs_enabled` env-var setting (default off) + NEW `_compute_logprob_confidence(response)` helper in `systems/executor.py` (extracts `response.raw['logprobs'][0]['logprob']`, returns `exp(lp)` or None) + MODIFIED `classify_llm_router_edges` to prefer logprob-based confidence when env-var on + logprobs present, fall back to W33 [IMPROVE-167] heuristic `1 / matched_count` otherwise. 7 NEW behaviour tests across providers + classifier (settings-shape / settings-from-dict / provider-passes-logprobs / provider-omits-logprobs / classifier-uses-logprobs / classifier-falls-back-on-missing / classifier-falls-back-on-disabled). | +7 |
| 3 | (doc)         | pending | Wave 42 end-wave retrospective. Flips Wave 42 status (in progress → ✓ shipped). Adds ✓ prefix on §10.4 IMPROVE-179 row. NEW Wave 42 architectural impact subsection. | 0 |
...
Every major feature of the Local AI Platform is now documented end-to-end, with **179** research-backed improvement ideas cross-referenced into one prioritized plan. Waves 1-24 + Wave 26 + Wave 27 + Wave 28 + Wave 29 + Wave 30 + Wave 31 + Wave 32 + Wave 33 + Wave 34 + Wave 35 + Wave 36 + Wave 37 + Wave 38 + Wave 39 + Wave 40 + Wave 41 fully shipped + Wave 25 deferred-by-investigation + Wave 42 in progress; post-Wave-42 backlog in deferred queue.
