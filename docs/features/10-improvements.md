...
### Wave 45 — Phase 1 cleanup wave 3 of 3 (final): Test-infrastructure consolidation (in progress 2026-05-06)
...
| # | Tag | SHA | What landed | Tests |
|---|---|---|---|---:|
| 1 | (doc)         | this    | Wave 45 mid-wave (start) — register Wave 45 in §10.5 + §10.6 with the Test-infrastructure consolidation design + Phase 1 cleanup wave 3 of 3 (final) framing per §10.5.2 closure plan + audit-revised scope (top-2 not top-3). Updates §10.1 wave-status + §10.4 reservation row for IMPROVE-185. | 0 |
| 2 | [IMPROVE-185] | TBD     | NEW `tmp_db` + `tmp_editor_env` fixtures in `tests/conftest.py`. MODIFIED 6 consumer test files to drop their local fixture definitions. NEW `tests/test_conftest_shared_fixtures.py` pinning the shared fixtures' behaviour. | TBD |
| 3 | (doc)         | TBD     | Wave 45 end-wave retrospective + Phase 1 close summary. Flips Wave 45 status (in progress → ✓ shipped). Adds ✓ prefix on §10.4 IMPROVE-185 row. NEW Wave 45 architectural impact subsection. NEW Phase 1 close note. | 0 |

Wave 45 is wave 3 of 3 in the §10.5.2 closure plan
Phase 1 cleanup arc — the FINAL Phase 1 wave. Single-
numbered shape: 3 commits (1 mid-doc + 1 numbered + 1
retro). After W45 ships, Phase 1 closes; Phase 2
capstone audits (Waves 46-50) follow only after explicit
user pause.
...
| # | Tag | SHA | What landed | Tests |
|---|---|---|---|---:|
| 1 | (doc)         | this    | Wave 45 mid-wave (start) — register Wave 45 in §10.5 + §10.6 with the Test-infrastructure consolidation design + Phase 1 cleanup wave 3 of 3 (final) framing per §10.5.2 closure plan + post-Wave-44 backlog promotion + audit-revised scope (top-2 not top-3). Updates §10.1 wave-status + §10.4 reservation row for IMPROVE-185. | 0 |
| 2 | [IMPROVE-185] | TBD     | NEW `tmp_db` + `tmp_editor_env` fixtures in `tests/conftest.py`. MODIFIED 6 consumer test files (4 `tmp_db` + 2 `tmp_editor_env`) to drop their local fixture definitions and inherit from conftest. LEFT LOCAL: `test_db_pragmas.py:tmp_db` (distinct shape) + 3 `_bare_engine`-named fixtures across partner tests (3 different shapes). NEW `tests/test_conftest_shared_fixtures.py` pinning the shared fixtures' behaviour. | TBD |
| 3 | (doc)         | TBD     | Wave 45 end-wave retrospective + Phase 1 close summary. Flips Wave 45 status (in progress → ✓ shipped). Adds ✓ prefix on §10.4 IMPROVE-185 row. Fills in mid-wave + numbered SHA placeholders. NEW Wave 45 architectural impact subsection. NEW Phase 1 close note. | 0 |

Wave 45 is wave 3 of 3 in the §10.5.2 closure plan
Phase 1 cleanup arc — the FINAL Phase 1 wave. Single-
numbered shape: 3 commits.
